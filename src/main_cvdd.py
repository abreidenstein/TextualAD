...
from utils.misc import print_text_samples, print_top_words, get_correlation_matrix, print_text_samples_with_scores
...
@click.option('--net_name', type=click.Choice(['cvdd_Net']))
...
# Modification: adding embedding choice
@click.option('--pretrained_model', default=None,
              type=click.Choice([None, 'GloVe_6B', 'GloVe_42B', 'GloVe_840B', 'GloVe_twitter.27B', 'FastText_en','FastText_fr',
                                 'bert', 'FastText_dataset', 'FastText_class', 'PPMI_dataset', 'PPMI_class']),
...
@click.option('--lr_milestone', type=int, default=40, #multiple=True,
...
def main(dataset_name, net_name, xp_path, data_path, results_file, load_config, load_model, device, seed, tokenizer, clean_txt,
    ...
    loggerSimp.info('Network: %s' % net_name)
    loggerSimp.info('Pre-trained model: %s' % cfg.settings['pretrained_model'])
    loggerSimp.info('Number of attention heads: %d' % cfg.settings['n_attention_heads'])
    ...
    dataset = load_dataset(dataset_name, data_path, normal_class, cfg.settings['tokenizer'],
                           clean_txt=cfg.settings['clean_txt'], cont=cont)
    ...
    cvdd.train(dataset,
               ...
               lr_milestones=[str(cfg.settings['lr_milestone'])],
    ...
    print_text_samples_with_scores(dataset.test_set, dataset.encoder, idx_sorted.tolist(), scores, sort_idx,
