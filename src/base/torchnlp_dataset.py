...
class TorchnlpDataset(BaseADDataset):
    ...
    def loaders(self, batch_size: int, shuffle_train=False, shuffle_test=False, num_workers: int = 0) -> (
        ...
        train_sampler = BucketBatchSampler(self.train_set, batch_size=batch_size, drop_last=False, # True in code of Ruff et. al
                                           sort_key=lambda r: len(r['text']))
        test_sampler = BucketBatchSampler(self.test_set, batch_size=batch_size, drop_last=False, # True in code of Ruff et. al
                                          sort_key=lambda r: len(r['text']))
