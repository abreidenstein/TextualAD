...
class Newsgroups20_Dataset(TorchnlpDataset):
    ...
    def __init__(self, root: str, normal_class=0, tokenizer='spacy', use_tfidf_weights=False, append_sos=False,
        ...
        # Modification: adding a dataset name attribute
        self.dataset_name = 'newsgroup20'
        self.dataset_name_files = '20ng_od'
        self.cont = cont
        classes = list(range(6))

        # Modification: adding different ways of considering categories
        ...
        # Modification: different ways of keeping track of normal class
        self.normal_class = normal_class
        self.normal_classes = ag_subsets[normal_class]
        self.outlier_classes = []
        del classes[normal_class]
        for i in classes:
            # Modification: same for outlier classes
            self.outlier_classes += ag_subsets[i]

        # Load the 20 Newsgroups dataset
        # Modification: adding cont argument
        self.train_set, self.test_set = newsgroups20_dataset(directory=root, train=True, test=True, clean_txt=clean_txt, cont=cont)
        ...
        for i, row in enumerate(self.train_set):
            # Modification: from new normal_classes
            if row['label'] == self.normal_classes:
        ...
        for i, row in enumerate(self.test_set):
            # Modification: from new normal_classes
            row['label'] = torch.tensor(0) if row['label'] == self.normal_classes else torch.tensor(1)
            row['text'] = row['text'].lower()
...
# Modification: adding cont argument + completely adapting the function to new way of handling datasets
def newsgroups20_dataset(directory='../data', train=False, test=False, clean_txt=False, cont=None):
    """
    Load the 20 Newsgroups dataset.
