...
class LanguageModelingModel:
    def __init__(
        ...
        train_files=None,
        trainFilesForTokenizer = None, #added to be able to train an electra tokenizer on the dataset fo RNCP (as it is in french)
        args=None,
        use_cuda=True,
        cuda_device=-1,
        **kwargs,
        ...
        Args:
            ...
            trainFilesForTokenizer (optional): used if you want to train a tokenizer on a specific dataset. This file may be different than train_files if you want the tokenizer to be trained on the whole dataset and not just on one class.
        ...
        loggerSimp.info('Nb of masks: %s' % len(self.masks))
        ...
        if self.args.tokenizer_name:
            ...
            if not train_files:
                ...
                print("train_files", trainFilesForTokenizer)
                self.train_tokenizer(trainFilesForTokenizer)
                new_tokenizer = True
    ...
    def train_model_anomaly(
        ...
        trainFilesForSaving=None, # files used to learn representations when we want them to be saved
        verbose=True,
        **kwargs,
    ):
        """
        ...
        Args:
            ...
            args (optional): Optional changes to the args dict of the model. Any changes made will persist for the model.
            eval_file (optional): Path to eval file containing the text to evaluate the language model on.
            trainFilesForSaving (optional): files used to learn representations when we want them to be saved
        ...
        eval_doc_dataset_outliers = DocumentDataset(
            ...
            2,
            sliding_window=self.args.sliding_window)
        trainDatasetForSaving = DocumentDataset( # dataset used to learn representations when we want them to be saved
            self.tokenizer,
            self.args,
            trainFilesForSaving,
            self.args.block_size,
        ...
        global_step, tr_loss = self.train_anomaly(
            ...
            trainDatasetForSaving, # dataset used to learn representations when we want them to be saved
            show_running_loss=show_running_loss,
            verbose=verbose,
            **kwargs,
        )
        ...
        # to evaluate the model in the end
        self.eval_model_document(
                            eval_doc_dataset,
                            eval_doc_dataset_outliers,
        ...
        with torch.no_grad():
            ...
            if not ("vanilla_electra" in self.extra_args and self.extra_args["vanilla_electra"] != False):
                ...
                for mask_idx, mask_ in enumerate(masks):
                    ...
                    out_softmax = F.softmax(d_output,dim=0).cpu().numpy()
    ...
    def train_anomaly(
        ...
        **kwargs,
        ...
        if self.extra_args['random_generator'] == False:
            ...
            loggerSimp.info('Loss : %s' % len(self.masks))

        if (self.extra_args['use_rtd_loss'] == True) and (self.extra_args['rmd_loss_weight']!=0)and (self.extra_args['rtd_loss_weight']!=0):
            logger.info(' USING RTD LOSS')
            loggerSimp.info('Loss : RMD + RTD') # the normal DATE algorithm uses the RMD loss, without it it corresponds to using an ELECTRA model trained on the data
        elif (self.extra_args['use_rtd_loss'] == True) and (self.extra_args['rmd_loss_weight']==0)and (self.extra_args['rtd_loss_weight']!=0):
            loggerSimp.info('Loss : RTD')
        else:
            loggerSimp.info('Loss : RMD')
        ...
        for current_epoch in train_iterator:
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                # the following block was commented to simplify the launch of multiple experiments and avoid evaluating at step 0
                # if global_step == 1:
                # 
        ...
        # in comparison with Manolache's code, this blok was moved here to exctrat representations even if we do not evaluate during training
        if self.extra_args['extract_repr_for_OCSVM'] == True and self.extra_args['dataset_repr'] == True:
            self.extractRepr(collate,
                        tokenizer, masks, args,
                        model,
                        trainDatasetForSaving, # dataset used to learn representations when we want them to be saved
                        eval_doc_dataset,eval_doc_dataset_outliers,
                        dataset_reprs = True)
        elif self.extra_args['extract_repr_for_OCSVM']:
            self.extractRepr(collate,
                        tokenizer, masks, args,
                        model,
                        train_dataset,
    ...
    # This function saves DATE representations to be used by OCSVM for experimental purposes
    # Here we use the [CLS] token to get directly a contextual document representation
    # We have to get and save representations for the training and testing set.
    def extractRepr(self,
                     collate,
                     tokenizer, masks, args,
                     model,
                     train_dataset,
        ...
        with torch.no_grad():
            ...
            train_reprs = []
            train_sampler_repr = SequentialSampler(train_dataset)
            train_dataloader_repr = DataLoader(train_dataset,
                                        batch_size=args.train_batch_size,
                                        sampler=train_sampler_repr,
                                        collate_fn=collate,
            ...
            train_reprs = []
            i = 0
            for batch in train_dataloader_repr:
                mask_repr = {
            ...
            labels = []
            label = 0
            for evalDataset in [eval_doc_dataset,eval_doc_dataset_outliers]:
                test_sampler_repr = SequentialSampler(evalDataset) #if args.local_rank == -1 else DistributedSampler(testFilesForTokenizer) #RandomSampler(train_dataset) if args.local_rank == -1 else DistributedSampler(train_dataset)
                test_dataloader_repr = DataLoader(evalDataset,
                                        batch_size=args.train_batch_size,
                                        sampler=test_sampler_repr,
                                        collate_fn=collate,
                                        num_workers=4)
                idx_list = test_dataloader_repr.dataset.idx_l
                self.idx_list = idx_list
                first = True
                for batch in test_dataloader_repr:
                    if first == True:
                ...
                testRepr = self.repr_list
                for reprs in testRepr:
                    if len(reprs) > 1:
                        meanRepr = np.array(np.mean(reprs, axis = 0).reshape(-1,))
                        finalTestReprs.append(meanRepr)
                    else:
                        meanRepr = np.array(reprs).reshape(-1,)
                        finalTestReprs.append(meanRepr)
                    labels.append(label)
                label = 1
            
            finalTestReprs = np.array(finalTestReprs)

            
            
            # train representations
            _,_,datasetName,train,cont,fileName = train_dataset.file_path.split("/")
            if dataset_reprs:
                reprFile = "../data/word_vectors_saved/"+datasetName+"/train/DATE_Dataset"
            else :
                reprFile = "../data/word_vectors_saved/"+datasetName+"/train/"+cont+"/DATE_"+fileName[:-4]
            if self.extra_args['rmd_loss_weight']==0:
                reprFile += "_noRMD"
            directory = os.path.dirname(reprFile)
            if not os.path.exists(directory):
                os.makedirs(directory)
            Path(reprFile).touch()
            with open(f'{reprFile}', 'wb') as fp:
                pkl.dump(train_reprs, fp)
            
            # test representations
            # we have a dataset representation for each class in testing as the labels change and they are saved each time with the representations of the testing set
            if dataset_reprs:
                reprFile = "../data/word_vectors_saved/"+datasetName+"/test/DATE_Dataset_"+fileName[:-4]
            else :
                reprFile = "../data/word_vectors_saved/"+datasetName+"/test/DATE_"+fileName[:-4]
            if self.extra_args['rmd_loss_weight']==0:
                reprFile += "_noRMD"
            directory = os.path.dirname(reprFile)
            if not os.path.exists(directory):
    ...
    def evaluate_anomaly_docs(self,
        ...
        if self.extra_args['eval_anomalies']:
            ...
            # In the article of Manolache et al, the PL score was said to be the best, so the other ones got commented
            ...
            loggerSimp.info('Test AUC: {:.10f}'.format(100. * results["PL_RTD/roc_auc"]))
            loggerSimp.info('Test AUPR-in: {:.10f}'.format(100. * results["PL_RTD/pr_auc_in"]))
            loggerSimp.info('Test AUPR-out: {:.10f}'.format(100. * results["PL_RTD/pr_auc_out"]))
    ...
    def get_preds_calc_metrics(self,
        ...
        if any(np.isnan(anomaly_scores)):
            anomaly_scores = anomaly_scores[~np.isnan(anomaly_scores)]
        if any(np.isnan(normal_scores)):
        ...
        nsIsnan = np.isnan(normal_scores)
        asIsnan = np.isnan(anomaly_scores)
