...
class CVDDTrainer(BaseTrainer):
    ...
    def train(self, dataset: BaseADDataset, net: CVDDNet):
        ...
        for epoch in range(self.n_epochs):
            ...
            for data in train_loader:
                ...
                if len(text_batch.shape) == 1:
                    text_batch = torch.unsqueeze(text_batch, 0)
    ...
    def test(self, dataset: BaseADDataset, net: CVDDNet, ad_score='context_dist_mean'):
        ...
        logger.info('Test AUC: {:.10f}%'.format(100. * self.test_auc))
        logger.info('Test AUPR-in: {:.10f}%'.format(100. * self.test_auprIn))
        loggerSimp.info('Test AUPR-in: {:.10f}'.format(100. * self.test_auprIn))
        logger.info('Test AUPR-out: {:.10f}%'.format(100. * self.test_auprOut))
        loggerSimp.info('Test AUPR-out: {:.10f}'.format(100. * self.test_auprOut))
        logger.info(f'Test Best Context: {best_context}')
        logger.info('Test Time: {:.3f}s'.format(self.test_time))
        logger.info('Finished testing.')

        #log simplifié
        loggerSimp.info('Test AUC: {:.10f}'.format(100. * self.test_auc))
        loggerSimp.info('------------')
